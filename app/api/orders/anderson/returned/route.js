...
/**
 * GET /api/orders/anderson/returned
 * Get returned orders (admin only)
 */
export async function GET(request) {
  try {
    ...
    if (startDate || endDate) {
      filter.return_date = {};
      if (startDate) filter.return_date.$gte = new Date(startDate);
      if (endDate) filter.return_date.$lte = new Date(new Date(endDate).setHours(23, 59, 59, 999));
    }
    ...
    console.error('[API] Error fetching returned orders:', error);
    return NextResponse.json(
