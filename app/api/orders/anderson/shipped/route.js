...
/**
 * GET /api/orders/anderson/shipped
 * Get orders in transit (admin only)
 */
export async function GET(request) {
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate) filter.shipping_date.$gte = new Date(startDate);
      if (endDate) filter.shipping_date.$lte = new Date(new Date(endDate).setHours(23, 59, 59, 999));
    }

    const orders = await Order.find(filter)
      .populate('wilaya_id', 'name code')
      .populate('commune_id', 'name')
      .sort({ shipping_date: -1 })
      .skip(skip)
      .limit(limit)
      .lean();
    ...
    console.error('[API] Error fetching shipped orders:', error);
    return NextResponse.json(
